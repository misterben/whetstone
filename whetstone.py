...
class MVDB():
	...
	def verse_list(self, catref=0):
		...
		if (catref != 0):
			self.cur.execute("SELECT verseref, refno FROM verses WHERE catref=? AND verseref != 'END'",(catref,))
		else:
			self.cur.execute("SELECT verseref, refno FROM verses WHERE verseref != 'END'")
	...
	def get_verselist_todate(self, planner, date='now', limit = 0):
		...
		if (limit == 0):
			self.cur.execute("SELECT verseref FROM planner WHERE name = '"+planner+"' AND date <= ('"+date+"') AND verseref != 'END' ORDER BY date DESC")
		else:
			self.cur.execute("SELECT verseref FROM planner WHERE name = '"+planner+"' AND date <= ('"+date+"') AND verseref != 'END' ORDER BY date DESC LIMIT "+str(limit))
